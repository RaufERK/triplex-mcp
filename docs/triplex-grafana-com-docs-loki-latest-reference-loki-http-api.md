# triplex-grafana-com-docs-loki-latest-reference-loki-http-api

Source: https://grafana.com/docs/loki/latest/reference/loki-http-api/

---

[Documentation](/docs/)![breadcrumb arrow](/static/assets/img/icons/grafana-icon-breadcrumb-arrow-gray.svg) [Grafana Loki](/docs/loki/latest/)![breadcrumb arrow](/static/assets/img/icons/grafana-icon-breadcrumb-arrow-gray.svg) [Reference](/docs/loki/latest/reference/)![breadcrumb arrow](/static/assets/img/icons/grafana-icon-breadcrumb-arrow-gray.svg) Loki HTTP API
...
* [GET /loki/api/v1/query\_range](#query-logs-within-a-range-of-time)
...
* [GET /loki/api/v1/index/volume\_range](#query-log-volume)
...
* [GET /log\_level](#change-log-level)
...
### Flush/shutdown endpoints[ ](#flushshutdown-endpoints)
...
* [POST /ingester/prepare\_shutdown](#prepare-ingester-shutdown)
...
### Log deletion endpoints[ ](#log-deletion-endpoints)
...
* [GET /loki/api/v1/format\_query](#format-a-logql-query)
...
### Matrix, vector, and stream[ ](#matrix-vector-and-stream)
...
## Ingest logs using OTLP[ ](#ingest-logs-using-otlp)
...
For information on how to configure Loki, refer to the[OTel Collector topic](/docs/loki/latest/send-data/otel/).
...
If your cluster has[Grafana Loki Multi-Tenancy](../../operations/multi-tenancy/) enabled, set the `X-Scope-OrgID` header to identify the tenant you want to query. Here is the same example query for the single tenant called `Tenant1`:
...
## Query logs within a range of time[ ](#query-logs-within-a-range-of-time)
...
## Query label values[ ](#query-label-values)
...
## Query log statistics[ ](#query-log-statistics)
...
## Query log volume[ ](#query-log-volume)
...
## Patterns detection[ ](#patterns-detection)
...
`/metrics` returns exposed Prometheus metrics. See[Observing Loki](../../operations/meta-monitoring/)for a list of exported metrics.
...
## Flush in-memory chunks to backing store[ ](#flush-in-memory-chunks-to-backing-store)
...
## Prepare ingester shutdown[ ](#prepare-ingester-shutdown)
...
This API endpoint is usually used by Kubernetes-specific scale down automations such as the[rollout-operator](https://github.com/grafana/rollout-operator).

## Flush in-memory chunks and shut down[ ](#flush-in-memory-chunks-and-shut-down)
...
`/ingester/shutdown` triggers a shutdown of the ingester and notably will _always_ flush any in memory chunks it holds. This is helpful for scaling down WAL-enabled ingesters where we want to ensure old WAL directories are not orphaned, but instead flushed to our chunk backend.
...
## Index gateway ring status[ ](#index-gateway-ring-status)
...
### Request log deletion[ ](#request-log-deletion)
...
Create a new delete request for the authenticated tenant. The [log entry deletion](../../operations/storage/logs-deletion/) documentation has configuration details.
...
### List log deletion requests[ ](#list-log-deletion-requests)
...
List the existing delete requests for the authenticated tenant. The [log entry deletion](../../operations/storage/logs-deletion/) documentation has configuration details.
...
Remove a delete request for the authenticated tenant. The [log entry deletion](../../operations/storage/logs-deletion/) documentation has configuration details.
